...
function AddToTrace(span_metadata = {}) {
    return (target, propertyName, descriptor) => {
        ...
        const originalMethod = descriptor.value;
        descriptor.value = function (...args) {
            const { beeline } = this.ctx || findContextWithBeelineFrom(args) || {};
            if (beeline) {
                const return_type = Reflect.getMetadata('design:returntype', target, propertyName);
                const spanContext = {
                    'origin.type': 'decorator',
                ...
                return beeline.bindFunctionToTrace(() => {
                    return beeline[(return_type === null || return_type === void 0 ? void 0 : return_type.name) === 'Promise' ? 'withAsyncSpan' : 'withSpan'](spanContext, wrapped);
                });
            }
            else {
                throw new Error(`Cannot find an RumbleshipContext. Two solutions: 
